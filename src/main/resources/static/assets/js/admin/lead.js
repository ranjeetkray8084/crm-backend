...
async function loadLeads(passedCompanyId, page = 0) {
  ...
  try {
    const response = await fetch(`/api/companies/${companyId}/leads/admin-visible/${adminId}?page=${page}&size=${pageSize}`, {
      method: 'GET',
    ...
    const generateLeadRow = (lead) => {
      ...
      const selectStyle = `background-color: ${
        lead.status === 'NEW' ? '#f0ad4e' :
...
function leadloadCreatedByOptions() {
  ...
  // ✅ Fetch users
  fetch(`/api/users/companies/${companyId}/users/under-admin/${currentUserId}`, {
    method: 'GET',
    headers: { 'Content-Type': 'application/json' },
    credentials: 'include',
  })
...
async function filterLeads(page = 0) {
  ...
  try {
    ...
    const response = await fetch(`/api/companies/${companyId}/leads/visible-to-admin/${adminId}/search?${params.toString()}`, {
      method: 'GET',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include'
    });
...
async function showAssignmentModal(leadId) {
  try {
    ...
    // ✅ Updated API
    const response = await fetch(`/api/users/admin/${adminId}/users`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
