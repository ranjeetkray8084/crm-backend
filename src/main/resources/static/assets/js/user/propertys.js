      ...
      <div class="addproperty" style="max-height: 500px; overflow-y: auto; border: 1px solid #ccc; padding: 15px;">
        <form id="addPropertyForm" onsubmit="saveProperty(event)">
          ...
          <div>
            ...
            <select id="type" name="type" required onchange="toggleBhkField();">
...
//   priceLabel.textContent = (typeSelect.value === 'Commercial') ? 'Lease Amount:' : 'Price:';
// }
...
async function saveProperty(event) {
  ...
  console.log("Floor value:", document.getElementById("floor").value);
...
function renderPropertyPaginationControls(companyId, totalPages, currentPage) {
  ...
  container.innerHTML = `
    <button onclick="loadProperty(${companyId}, ${currentPage - 1})" ${currentPage === 0 ? 'disabled' : ''}>Previous</button>
    <span style="margin: 0 10px;">Page ${currentPage + 1} of ${totalPages}</span>
    <button onclick="loadProperty(${companyId}, ${currentPage + 1})" ${currentPage >= totalPages - 1 ? 'disabled' : ''}>Next</button>
  `;
}
...
function handleTagInput(event) {
  ...
  if ((event.key === 'Enter' || event.key === ',') && value) {
    ...
    if (!tags.includes(value)) {
      tags.push(value);
      renderTags();
      filterProperties(); // 🔁 Trigger filter on tag add
    }
    input.value = '';
  }
}
...
function removeTag(index) {
  tags.splice(index, 1);
  renderTags();
  filterProperties(); // 🔁 Trigger filter on tag remove
}
...
function getPropertyFilterValue(id) {
  return (
    document.getElementById("desktop-" + id)?.value?.trim() ||
    document.getElementById("mobile-" + id)?.value?.trim() ||
    ''
  );
}
...
/* ------------------------------------------------------------------------ */
/* 1) ADD‑REMARK MODAL (optional propertyName param for nicer UX)           */
