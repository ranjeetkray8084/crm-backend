...
async function loadProperty(passedCompanyId, page = 0) {
  ...
  try {
    ...
    if (!companyId || isNaN(userId)) {
      console.error("âŒ Missing companyId or userId");
    ...
    for (const property of properties) {
      try {
        ...
        const rowColor = property.status === "AVAILABLE"
          ? "#e8f5e9"
        ...
        row.innerHTML = `
          ...
          <td>${safeDisplay(property.unitDetails)}</td>
          <td>${safeDisplay(property.floor)}</td>
          <td>${safeDisplay(property.size)}</td>
          <td>${safeDisplay(property.sector)}</td>
          <td>${safeDisplay(property.location)}</th>
          ...
          <td>${safeDisplay(property.ownerContact)}</td>
        ...
        console.error("âš ï¸ Error rendering property row:", rowError);
      }
    }

    renderPropertyPaginationControls(companyId, totalPages, propertyCurrentPage);
  } catch (error) {
    console.error("âŒ Error loading properties:", error);
...
async function filterProperties(page = 0) {
  ...
  try {
    ...
    const generateRow = (p) => {
      const createdOn = p.createdAt
        ? `${new Date(p.createdAt).toLocaleDateString()} ${new Date(p.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`
        : 'N/A';

      let sourceDisplay = safeDisplay(p.source);
      if (p.source === "Reference" && p.referenceName) {
        sourceDisplay += ` (${safeDisplay(p.referenceName)})`;
      }

      return `
        <tr>
          <td>${safeDisplay(p.propertyName)}</td>
          <td>${safeDisplay(p.type)}</td>
          <td>${safeDisplay(p.bhk)}</td>
          <td>${safeDisplay(p.unitDetails)}</td>
          <td>${safeDisplay(p.floor)}</td>
          <td>${safeDisplay(p.size)}</td>
          <td>${safeDisplay(p.sector)}</td>
          <td>${safeDisplay(p.location)}</td>
          <td>${safeDisplay(p.status)}</td>
          <td>${formatToINR(p.price)}</td>
          <td>${sourceDisplay}</td>
          <td>
            <button onclick="openAddPropertyRemarkModal(${p.propertyId})">Add</button>
            <button onclick="viewPropertyRemarks(${p.propertyId})">View</button>
          </td>
          <td>${safeDisplay(p.ownerName)}</td>
          <td>${safeDisplay(p.ownerContact)}</td>
          <td>${safeDisplay(p.createdBy?.name || p.createdByName)}</td>
          <td>${createdOn}</td>
          <td>
            <button onclick="handleShowUpdateProperty(this)"
                    data-property='${encodeURIComponent(JSON.stringify(p))}'
                    data-company-id="${companyId}">Update</button>
            <button onclick="deleteProperty(${p.propertyId})">Delete</button>
          </td>
        </tr>
      `;
    };
    ...
    tbody.innerHTML = properties.length > 0
      ? properties.map(generateRow).join("")
      : `<tr><td colspan="16" style="text-align:center">No properties found.</td></tr>`;
