...
function setupFormEvents() {
  ...
  visibilitySelect.addEventListener('change', async () => {
    const visVal = visibilitySelect.value;

    if (visVal === 'SPECIFIC_USERS') {
      specificUsersSection.style.display = 'block';
      await loadUsersForNotes();
    } else if (visVal === 'SPECIFIC_ADMIN') {
      specificUsersSection.style.display = 'block';
      await loadAdminsForNotes();
    } else {
...
-------------------------------------------------------------- */
async function loadAdminsForNotes() {
  ...
  try {
    ...
    const users = (await res.json()).filter(u => u.role === 'ADMIN');
    ...
    container.innerHTML = users.length
      ...
      : '<p style="color:#ccc;">No admins found.</p>';

  } catch (e) {
    console.error('Failed to load admins:', e);
    container.innerHTML = '<p style="color:red;">Error loading admins.</p>';
  }
}
...
async function saveNote() {
  ...
  if (visibility === 'SPECIFIC_USERS' || visibility === 'SPECIFIC_ADMIN') {
    ...
    if (visibleUserIds.length === 0) {
      customAlert('Select at least one user/admin.');
      btn.disabled = false;
      return;
    }
  }
...
async function loadNotes() {
  ...
  const endpoints = {
    directorNotesUrl: `/api/companies/${companyId}/notes/director-visible/${userId}`,
    getUsernameUrl: (id) => `/api/users/${id}/username`
  };

  try {
    const res = await fetch(endpoints.directorNotesUrl, {
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include'
    });

    if (!res.ok) throw new Error("Failed to fetch notes.");
    const allNotesRaw = await res.json();
